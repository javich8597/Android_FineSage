...
class FinanceViewModel(context: Context) : ViewModel() {
    ...
    val missions: StateFlow<List<com.example.data.model.Mission>> = repository.allMissions
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())
    ...
    private val _currentAuditText = MutableStateFlow<String?>(null)
    val currentAuditText: StateFlow<String?> = _currentAuditText.asStateFlow()


    // --- Pet Config ---
    private val _petStyle = MutableStateFlow("")
    ...
    val petMood: StateFlow<PetMood> = combine(transactions, goals) { txList, goalList ->
        VirtualPetEngine.calculateMood(txList, goalList)
    }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), PetMood.ANGRY)

    val petXp: StateFlow<Int> = combine(transactions, goals, missions) { txList, goalList, missionList ->
        VirtualPetEngine.calculateXp(txList, goalList, missionList)
    }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), 0)

    val petLevel: StateFlow<Int> = petXp.map { xp ->
        VirtualPetEngine.calculateLevel(xp)
    }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), 1)

    val petXpInLevel: StateFlow<Int> = petXp.map { xp ->
        VirtualPetEngine.calculateXpInLevel(xp)
    }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), 0)
    ...
    fun requestAuditAndMissions() {
        if (_isCoachingLoading.value) return
        _isCoachingLoading.value = true
        viewModelScope.launch(Dispatchers.IO) {
            val txList = transactions.value
            val result = GeminiApiClient.generateMissionsAndAudit(txList)
            if (result != null) {
                _currentAuditText.value = result.first
                val generatedMissions = result.second
                repository.insertMissions(generatedMissions)
            } else {
                _currentAuditText.value = "Hubo un error al conectar con la IA."
            }
            _isCoachingLoading.value = false
        }
    }
    private val _goalAdviceText = MutableStateFlow<String?>(null)
    val goalAdviceText: StateFlow<String?> = _goalAdviceText.asStateFlow()

    private val _isGoalAdviceLoading = MutableStateFlow(false)
    val isGoalAdviceLoading: StateFlow<Boolean> = _isGoalAdviceLoading.asStateFlow()

    fun requestGoalAdvice(goal: BudgetGoal) {
        if (_isGoalAdviceLoading.value) return
        _isGoalAdviceLoading.value = true
        _goalAdviceText.value = null
        viewModelScope.launch(Dispatchers.IO) {
            val txList = transactions.value
            val advice = GeminiApiClient.generateGoalAdvice(goal, txList)
            if (advice != null) {
                _goalAdviceText.value = advice
            } else {
                _goalAdviceText.value = "Hubo un error al conectar con la IA."
            }
            _isGoalAdviceLoading.value = false
        }
    }
