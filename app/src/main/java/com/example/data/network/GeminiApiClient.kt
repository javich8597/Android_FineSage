...
object GeminiApiClient {
    ...
    suspend fun generateMissionsAndAudit(transactions: List<Transaction>): Pair<String, List<com.example.data.model.Mission>>? = withContext(Dispatchers.IO) {
        val apiKey = BuildConfig.GEMINI_API_KEY
        if (apiKey.isEmpty() || apiKey == "MY_GEMINI_API_KEY") {
            Log.w(TAG, "Gemini API key is not configured.")
            return@withContext null
        ...
        val historySummary = transactions.take(30).joinToString(", ") { "${it.concept} (${it.amount} EUR, ${it.category})" }

        try {
            val prompt = "Analiza estos gastos recientes: [$historySummary]. Devuelve un JSON puro sin markdown con esta estructura: {\"auditText\": \"Tu diagnstico breve de la situacin\", \"missions\": [{\"title\": \"Ttulo de la misin\", \"description\": \"Descripcin de la misin\", \"xpReward\": 500}]}. Dame 2 misiones que fomenten el ahorro o limiten el gasto en las categoras ms altas que veas."
            ...
            client.newCall(request).execute().use { response ->
                ...
                if (!answer.isNullOrEmpty()) {
                    ...
                    try {
                        val parsed = JSONObject(answer)
                        val auditText = parsed.optString("auditText", "Sin auditora disponible.")
                        val missionsJson = parsed.optJSONArray("missions")
                        val missionsList = mutableListOf<com.example.data.model.Mission>()
                        if (missionsJson != null) {
                            for (i in 0 until missionsJson.length()) {
                                val mJson = missionsJson.optJSONObject(i)
                                if (mJson != null) {
                                    missionsList.add(
                                        com.example.data.model.Mission(
                                            title = mJson.optString("title", "Misin de ahorro"),
                                            description = mJson.optString("description", "Ahorra dinero"),
                                            xpReward = mJson.optInt("xpReward", 500)
                                        )
                                    )
                                }
                            }
                        }
                        return@withContext Pair(auditText, missionsList)
                    } catch (e: Exception) {
                        Log.e(TAG, "Failed to parse missions JSON: $answer", e)
                        return@withContext null
                    }
                } else {
                    return@withContext null
            ...
            Log.e(TAG, "Exception calling Gemini API for missions: ${e.message}", e)
            return@withContext null
        }
    }

    suspend fun generateGoalAdvice(goal: BudgetGoal, transactions: List<Transaction>): String? = withContext(Dispatchers.IO) {
        val apiKey = BuildConfig.GEMINI_API_KEY
        if (apiKey.isEmpty() || apiKey == "MY_GEMINI_API_KEY") {
            Log.w(TAG, "Gemini API key is not configured.")
            return@withContext null
        ...
        val missingAmount = goal.targetAmount - goal.savedAmount
        val historySummary = transactions.take(30).joinToString(", ") { "${it.concept} (${it.amount} EUR)" }

        val prompt = "Tengo una meta financiera de ahorrar ${goal.targetAmount} EUR para ''. Me faltan $missingAmount EUR. Mis gastos recientes son: [$historySummary]. Dame un consejo super breve de 2 frases sobre cmo lograr esta meta antes recortando algn gasto recurrente de la lista. Habla directamente como un coach."
        ...
        try {
            ...
            client.newCall(request).execute().use { response ->
                ...
                return@withContext firstPart?.optString("text")?.trim()
            }
        } catch (e: Exception) {
            Log.e(TAG, "Exception calling Gemini API for goal advice: ${e.message}", e)
            return@withContext null
        }
    }
}